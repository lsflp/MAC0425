...
class ProgressionPlanning(object):
    ...
    def goal_test(self, state):
        ...
        return self._problem.goal.issubset(state)
    ...
    def solve(self, W, heuristics):
        ...
        while not frontier.is_empty():
            ...
            num_explored += 1
            if self.goal_test(set(node.state)):
                return (node.path(), num_explored, num_generated)
            actions = self.applicable(set(node.state))
            for action in actions:
                newState = self.successor(set(node.state), action)
                child = Node(tuple(newState), action, node, node.g+1, heuristics(newState, self))
